...
/* Yields to higher priority threads on the ready_list. Works in either
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->priority = t->base_priority = priority; /* P1-2 */
  t->magic = THREAD_MAGIC;

  /* P1-1 */
  /* 1. initialize sleep semaphore to 0 */
...
/*     return list_entry (list_pop_front (&ready_list), struct thread, elem); */
     ...
     /* Note: list_remove returns the following element
        ...
        likely to fail the is_thread(t) assertion in
        schedule(). */
...
/* DEBUG */ASSERT(next->magic == THREAD_MAGIC);
...
/* P1-2 */
/* Donates priority. */
void thread_donate_priority (struct lock *lock, struct thread *donor) { 
  ASSERT (lock != NULL);
  ASSERT (is_thread(donor));
  
  /* get the lock holder thread */
  struct thread *holder_t = lock->holder;
  
  /* set the blocking lock for the donor */
  donor->blocking_lock = lock;
  /* add donor thread to the donor_list of the holder */
  list_push_back (&holder_t->donor_list, &donor->donor_elem);
  /* call thread_update_priority with the holder */
  thread_update_priority (holder_t);
}

/* P1-2 */
/* Updates the (effective) priority to the highest
   donated priority or base_priority. Cascades as
   far as necessary. */
void thread_update_priority (struct thread *t) {
  ASSERT (is_thread(t));
 /* if the donor_list is empty, set priority = base_priority */
  ...
  if (t->blocking_lock) {
    struct thread *h = t->blocking_lock->holder;
    thread_update_priority (h);
  }
}

/* P1-2 */
/* Releases the priority donation related to the LOCK. */
void thread_release_donation (struct lock *lock) {
  struct list_elem *e;
  /* loop through the donor_list and remove the donors 
     blocking on the lock, setting their blocking_lock to NULL */
  ASSERT (lock);
  ASSERT (lock_held_by_current_thread (lock));

  struct thread *curr = thread_current ();
  for (e = list_begin (&curr->donor_list);
       e != list_end (&curr->donor_list); ) {
    struct thread *t = list_entry (e, struct thread, donor_elem);
    if (t->blocking_lock == lock) { /* also works for NULL */
      e = list_remove (e);
      t->blocking_lock = NULL;
    } else {
      e = list_next(e);
